...
// Mod wheel: vertical drag, range [0, 1], holds position
export default function ModWheel({ label = 'Mod', value = 0, onChange, height = 120 }) {
  ...
  function handlePointerDown(e) {
    ...
    const onMove = (ev) => {
      ...
      let nv = startV - dy / range; // 1 trackH = full range
  ...
  return (
    <div className="wheel" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div ref={ref} className="wheel-track" onPointerDown={handlePointerDown}
        ...
        <div style={{ position: 'absolute', left: '50%', top: 10 + (1 - pct) * trackH, transform: 'translate(-50%, -50%)', width: 28, height: 28, borderRadius: 14, background: 'radial-gradient(circle at 30% 30%, #86efac, #16a34a 60%, #0b1020)', boxShadow: '0 4px 10px rgba(0,0,0,0.5), inset 0 2px 6px rgba(255,255,255,0.2)' }} />
